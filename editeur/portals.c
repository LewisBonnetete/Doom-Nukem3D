...
/*   Updated: 2020/04/16 16:20:24 by lewis            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((w1->a.z > w2->d.z) || (w2->a.z > w1->d.z) || (w1->a.z == w2->a.z) || (w1->d.z == w2->d.z))
		return (1);
	return (0);
}
...
int		find_otherside(t_map *map, t_wall *portal)
{
	...
	while(sector)
	{
		...
		{
			if (portal->sector_id != i && is_same_wall(portal, &sector->walls[i]) && similar_height(portal, &sector->walls[i]))
			{
...
void		portal_test(t_map* map)
{
	...
	while(sector)
	{
		...
		while (i < sector->nbr_walls)
		{
			...
			{
				printf("%i -> %i:%i | %i:%i (%i-%i)\n", sector->sector_id, sector->walls[i].a.x, sector->walls[i].a.y, sector->walls[i].b.x, sector->walls[i].b.y, sector->walls[i].b.z, sector->walls[i].c.z);
			}
			i++;
		}
		sector = sector->next_sector;
