...
/*   Updated: 2020/06/26 14:55:30 by lbonnete         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_wall_intersections(t_point new, t_point old, t_wall wall)
{
	...
	if (new.x == old.x && w1.x != w2.x)
	{
		...
		if (is_between(its.y, (float)old.y, (float)new.y) && is_between(its.y, (float)w1.y, (float)w2.y))
			return(0);
		return (1);
	}
	...
	else
	{
		...
		if (is_between(its.x, (float)old.x, (float)new.x) && is_between(its.x, (float)w1.x, (float)w2.x))
			return (0);
	}
	return (1);
}